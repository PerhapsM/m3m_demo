...
DATA_DIR = Path(r"C:\Users\maoji\OneDrive\Personality\Workspace\Python\m3m_demo\Multispectral Photo Folder")
...
def build_dashboard():
    st.set_page_config(page_title="M3M Precision Agriculture Demo", layout="wide")

    st.title("DJI M3M Precision Agriculture Platform")
    st.caption("Mission Intelligence Dashboard · Multispectral Field Intelligence")
    ...
    with c1:
        st.metric("Mission", selected_mission)
    with c2:
        st.metric("Images", str(len(missions)))
    with c3:
        st.metric("Band Channels", ", ".join(sorted([*mission_files.keys()])))
    with c4:
        st.metric("Year", flight_year)

    st.markdown(
        """
        <div style="padding:12px;border-radius:10px;background:#ecfdf5;color:#064e3b;border:1px solid #86efac">
